<x-volunteer.dashboard-layout>
    ...
    <div class="flex flex-col gap-4 mt-10 tab-bg min-h-screen">
        ...
        <h2 class="text-4xl sm:text-5xl font-bold text-accent leading-tight text-center relative">
            Activities
            <svg class="absolute -bottom-2 left-1/2 transform -translate-x-1/2 w-32 h-3 text-primary/30" viewBox="0 0 100 12" fill="none">
        ...
        <div class="tabs tabs-lift">
            <label class="tab flex gap-1">
                ...
                <i data-lucide="timer" class="size-4 text-primary"></i>
                Time Tracking
            </label>
            <div class="tab-content bg-base-100 border-base-300 p-6">
                <div class="flex flex-col gap-8 items-center justify-center">
                    <div class="w-full max-w-2xl">
                        <div class="card shadow-2xl rounded-3xl bg-gradient-to-r from-accent/10 to-primary/10 p-8 text-center flex flex-col items-center">
                            <div class="flex items-center gap-4 mb-4">
                                <div class="bg-accent rounded-full p-3">
                                    <i data-lucide="timer" class="text-white size-8"></i>
                                </div>
                                <div>
                                    <h3 class="font-bold text-xl text-accent">Total Volunteer Hours</h3>
                                    <p class="mt-1 text-3xl font-semibold text-primary">100 Hours</p>
                                </div>
                            </div>
                            <div class="w-full bg-gray-200 rounded-full h-4 mt-4">
                                <div class="bg-gradient-to-r from-accent to-primary h-4 rounded-full" style="width: 80%"></div>
                            </div>
                            <p class="text-xs text-gray-500 mt-2">80% of your yearly goal</p>
                        </div>
                    </div>
                    <div class="w-full max-w-4xl">
                        <h4 class="font-bold text-accent mb-4 text-lg">Recent Activities</h4>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            @forelse($activities as $activity)
                                <div class="card bg-white rounded-2xl shadow-lg p-6 flex flex-col gap-2 hover:shadow-2xl transition-shadow">
                                    <div class="flex items-center gap-3 mb-2">
                                        <i data-lucide="activity" class="text-primary size-6"></i>
                                        <span class="font-bold text-accent text-lg">{{ $activity->name }}</span>
                                    </div>
                            ...
                            @empty
                                <div class="card rounded-2xl p-8 flex flex-col items-center justify-center gap-4 col-span-2">
                                    <h3 class="font-bold text-xl text-accent">No Activities Found</h3>
            ...
            <div class="tab-content bg-base-100 border-base-300 p-6">
                <div class="w-full">
                    <h4 class="font-bold text-accent mb-4 text-lg">Your Certificates</h4>
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                        @forelse ($certificates as $certificate)
                            <div class="card bg-white/90 rounded-2xl shadow-xl p-6 flex flex-col justify-between hover:shadow-2xl transition-shadow duration-300">
                                <div class="flex items-center gap-4 mb-4">
                                    <div class="bg-gradient-to-r from-accent to-primary rounded-full p-2">
                                        <i data-lucide="award" class="text-white size-8"></i>
                                    </div>
                                    <div>
                                        <h4 class="font-bold text-lg text-accent">{{ $certificate->name }}</h4>
            ...
            <div class="tab-content bg-base-100 border-base-300 p-6">
                <div class="flex flex-col gap-8 items-center">
                    <h4 class="font-bold text-accent mb-4 text-lg">Badges & Points</h4>
                            ...
                            <div class="card bg-white rounded-2xl shadow-lg p-6 flex flex-col gap-2 items-center hover:shadow-2xl transition-shadow">
                                <div class="flex items-center gap-3 mb-2">
                                    @if($badge->icon_file_id)
                                        ...
                                        @else
                                            <i data-lucide="star" class="text-primary size-8"></i>
                                        @endif
                                    @else
                                        <i data-lucide="star" class="text-primary size-8"></i>
                                    @endif
                                    <span class="font-bold text-accent text-lg">{{ $badge->name }}</span>
                                </div>
                                <p class="text-gray-700 text-sm text-center">{{ $badge->description }}</p>
                                <span class="badge bg-gradient-to-r from-accent to-primary text-white font-bold mt-2">{{ $badge->points }} Points</span>
                            </div>
                        @empty
                            <div class="card rounded-2xl p-8 flex flex-col items-center justify-center gap-4 col-span-3">
                    ...
                    <div class="w-full max-w-lg">
                        <div class="card shadow-xl rounded-2xl bg-gradient-to-r from-accent/10 to-primary/10 p-6 text-center">
                            <h3 class="font-bold text-lg bg-gradient-to-r from-accent to-primary bg-clip-text text-transparent">Total Points</h3>
                            <p class="mt-2 text-3xl font-semibold text-accent">{{ $totalBadgePoints }} Points</p>
                        </div>
                    </div>
                </div>
            </div>
