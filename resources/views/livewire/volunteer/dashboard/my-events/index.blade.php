        ...
        <div class="mb-6">
            <div class="flex flex-wrap gap-4 items-center">
                <div class="flex flex-wrap gap-3 flex-1">
                    ...
                    {{-- <button class="flex btn btn-ghost" wire:click="clear">
        ...
        <div class="border border-slate-200 overflow-hidden rounded-2xl shadow-sm">
            ...
            <div class="px-6 py-5 border-b border-slate-200 bg-slate-50">
                <div class="grid grid-cols-12 gap-4 text-sm font-semibold text-slate-700">
                    <div class="col-span-3">Event Details</div>
                ...
                <div class="hover:bg-white/60 transition-all duration-200 group" wire:key="{{ $item->id }}">
                    <div class="grid grid-cols-12 gap-4 items-center">
                        ...
                        <div class="col-span-1">
                            @if ($item->pivot && $item->pivot->status == 'accepted')
                            <span
                            ...
                            @elseif($item->pivot && $item->pivot->status == 'pending')
                            <span
                            ...
                            @elseif($item->pivot && $item->pivot->status == 'completed')
                            <span
                            ...
                            @elseif($item->pivot && ($item->pivot->status === 'rejected' || $item->pivot->status === 'cancelled'))
                            <span
                        ...
                        <div class="col-span-2">
                            <div class="flex items-center gap-2">
                                {{-- Only show download button if event has a signed contract --}}
                                @if($item->contractRequests && $item->contractRequests->isNotEmpty())
                                <button wire:click="downloadContract({{ $item->id }})"
                                    class="flex items-center gap-2 px-3 py-2 bg-blue-100 hover:bg-blue-200 text-blue-700 rounded-xl text-sm font-medium transition-all duration-200"
                                    wire:loading.attr="disabled"
                                    wire:target="downloadContract({{ $item->id }})">
                                    <i data-lucide="download" class="w-4 h-4"></i>
                                    <span wire:loading.remove wire:target="downloadContract({{ $item->id }})">Contract</span>
                                    <span wire:loading wire:target="downloadContract({{ $item->id }})">Downloading...</span>
                                </button>
                                @endif
                            </div>
                        </div>
