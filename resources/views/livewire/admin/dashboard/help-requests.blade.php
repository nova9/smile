                        ...
                        @forelse($tickets as $ticket)
                                        <tr class="border-t border-slate-100 hover:bg-slate-50/50 transition-colors duration-200">
                                            ...
                                            <td class="px-6 py-4 text-sm text-slate-600">
                                                {{ ucfirst(str_replace('_', ' ', $ticket->category)) }}</td>
                                            ...
                                            <td class="px-6 py-4 text-sm text-slate-600">{{ $ticket->created_at->format('M j, Y') }}
                                            </td>
                                            <td class="px-6 py-4">
                                        ...
                                        <dialog id="view_ticket_{{ $ticket->id }}" class="modal">
                                            <div
                                                class="modal-box w-11/12 max-w-2xl bg-white/90 backdrop-blur-sm border border-white/20 rounded-2xl">
                                                ...
                                                <div class="mb-6">
                                                    ...
                                                    <div class="bg-slate-50 p-4 rounded-xl border border-slate-100">
                                                        <div class="space-y-2">
                                                            <p class="text-sm"><span class="font-medium text-slate-600">Ticket
                                                                    ID:</span>
                                                                <span class="text-slate-900">#{{ $ticket->id }}</span>
                                                            </p>
                                                            <p class="text-sm"><span class="font-medium text-slate-600">User:</span>
                                                                <span
                                                                    class="text-slate-900">{{ $ticket->user_name ?? ($ticket->user->name ?? 'Unknown User') }}</span>
                                                            </p>
                                                            ...
                                                            <p class="text-sm"><span class="font-medium text-slate-600">Category:</span>
                                                                <span
                                                                    class="text-slate-900">{{ ucfirst(str_replace('_', ' ', $ticket->category)) }}</span>
                                                            </p>
                                                            ...
                                                            <p class="text-sm"><span class="font-medium text-slate-600">Status:</span>
                                                                <span
                                                                    class="text-slate-900">{{ ucfirst(str_replace('_', ' ', $ticket->status)) }}</span>
                                                            </p>
                                                            <p class="text-sm"><span class="font-medium text-slate-600">Date:</span>
                                                                <span
                                                                    class="text-slate-900">{{ $ticket->created_at->format('M j, Y') }}</span>
                                                            </p>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="modal-action flex gap-3">
                                                    @if($ticket->status !== 'resolved' && $ticket->status !== 'closed')
                                                        <button wire:click="chatWithUser({{ $ticket->id }})"
                                                            ...
                                                            <i data-lucide="message-circle" class="w-4 h-4 mr-2" wire:loading.remove
                                                                wire:target="chatWithUser({{ $ticket->id }})"></i>
                                                            <svg wire:loading wire:target="chatWithUser({{ $ticket->id }})"
                                                                class="animate-spin h-4 w-4 mr-2" xmlns="http://www.w3.org/2000/svg"
                                                            ...
                                                            <span wire:loading.remove wire:target="chatWithUser({{ $ticket->id }})">Chat
                                                                with User</span>
                                                            <span wire:loading
                                                                wire:target="chatWithUser({{ $ticket->id }})">Connecting...</span>
                                                        </button>
                                                    @else
                                                        <div class="px-4 py-2 bg-slate-100 text-slate-600 rounded-xl text-sm">
                                                            <i data-lucide="info" class="w-4 h-4 inline mr-2"></i>
                                                            Cannot chat - ticket is {{ $ticket->status }}
                                                        </div>
                                                    @endif
                                                    <button onclick="view_ticket_{{ $ticket->id }}.close()"
